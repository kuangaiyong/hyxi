...
> **当前状态（2026-09-19 重新实测）：之前的「IP 被整体封禁」是错的 —— Tweakers 抓得了。** `curl` 那类非浏览器客户端会被 DPG 的 WAF 403（响应里点名 ClientIP），但**真 Chrome 完全打得开**：浏览器的 TLS/JA3 与请求头过得去。2026-09-19 用真 Chrome 把串 `2336074` 全量跑通（3 页 203 楼、complete=true），真站因此是可验证、可重采的。原封禁结论当初是拿什么客户端试出来的已无从考证 —— **「站点打不开」这类结论必须写清是用什么客户端、拿到什么响应码**，否则下一个人会照着它白等一个月。仍然成立的那半条：连着打十几次同域请求会把这套 WAF 触发，采集节奏一个字都不能动。
