...
- **评论区的折叠要点开，否则那部分评论根本没进 DOM**（2026-08-04 实测一页上有 26 条这样的回复）。v1.11.0 起点，**真站上多数点下去是弹帖子浮层**（2026-09-13 实测，见下一节），怎么收割、怎么关、层级怎么算见 `Skill(hyxi-collector-semantics)`
...
`tests/fixtures/login_site.py` 的小组页按上述结构复刻，连坑一起：文本为空的头像链接排在作者链接前面、主贴 `aria-label` 给相对时间、评论 `aria-label` 给一个时区错开的绝对时间、tooltip 靠 `mouseover` 现挂、最后一条主贴的正文是折叠的（点「展开」才换成全文并把按钮变成「收起」）、中间夹一条没有链接也没有正文的广告 `article`、第一条评论是 3 段并列的 `div[dir=auto]` 且里面还嵌着一条回复。帖子浮层与固定链接页按下一节复刻（9004 / 9005）。

### 帖子浮层 DOM 实测结论（2026-09-13，用户授权的两次只读探查）

目标主贴 `2534929036982815`（原帖 6 条，v1.11.4 只采到 1 条）。原始记录在
`docs/fixes/2026-09-13-facebook-thread-incomplete.md`，**改评论提取之前先读这一段**：

- **卡片上**：评论数按钮 `[role=button]` 文字「6」、aria-label「发表评论」，**含回复的回复**；
  折叠叫「查看更多**回答**」（v1.11.x 的模式里没有「回答」，从来没被点过）；首屏那条评论的正文也挂着「展开」
- **点折叠 = pushState** 到 `/groups/<gid>/permalink/<id>/` + 弹 `[role=dialog]`（内容 XHR 取回）；
  **地址有的不带末尾斜杠**（2026-09-15 实测 `/permalink/2476737042802015`，比 id 要按路径段比）；
  浮层里 7 个 article = 主贴 + 6 条；点浮层 aria-label「关闭」的按钮后 URL 回到信息流、卡片还在、**没有重载**
- **浮层里回复不嵌在父评论的 article 里**：六条全是主贴 article 下的兄弟节点，层级只体现在缩进
  （`getBoundingClientRect().left` 418 / 479 / 521）和 DOM 深度上
- **链接**：评论 `?comment_id=<自己>`；回复不论第几层都是 `?comment_id=<所属顶层评论>&reply_comment_id=<自己>`；
  aria-label 是「评论者：X」「X回复了Y的评论」「X回复了Y的回复」（随界面语言变，不当判据）
- **浮层里的主贴头部没有 `/posts/<id>` 链接**
- **直接打开固定链接**是首页信息流上盖一层同样的浮层，背景里是不相干的帖子
- **评论排序默认「最相关」**，菜单三项「最相关 / 由新到旧 / 所有评论（含疑似垃圾信息）」，底部写着
  「评论排序方式将应用于 Facebook」—— **账号级设置，采集器不许点**。探查时切成过「所有评论」，
  按用户决定已于 2026-09-15 改回「最相关」。那次 Playwright 定位器点排序按钮 30 秒超时（什么都没改），
  改用 DOM `click()` 才点开 —— 与采集器点折叠同一种点法
