...
description: hyxi 采集语义：Facebook 评论区的折叠必须点开但有的会把页面带进帖子详情（留在那儿整轮只剩一条帖子）、正文图抓取（tweakers + facebook_group）、全量重跑 force_full、老帖新回复如何避免被时间倒序埋掉。改采集脚本的增量/去重/翻页逻辑之前读这份。
---
...
3. **要循环点到不再增长**（评论分页）。**一轮只展开一条主贴**（理由见下面 ⛔ 那一节），
...
修法三件事，改 `expandComments()` 之前逐条对一遍：

1. **一次只展开一条主贴的折叠**。一次点一整屏的话出了事根本不知道是哪一个，只能整批放弃
2. **点完检查是否还在信息流上**（`recoverFromDetailView()`）：换了 URL 就先 `goBack()`，
   回不去再重载；弹了 `[role=dialog]` 就 Esc。导航正在飞时 `page.$` 会抛
   「Execution context was destroyed」—— **那本身就是被带走的证据**，接住，别让它炸掉整轮
3. **带走过页面的主贴整轮拉黑（`detailPosts`，模块级），不能每批重建**。实测踩过：
   名单每批重建时，同一个坏按钮每批都点一遍、每次都重载信息流，把滚动加载出来的第二屏
   一起冲掉 —— 采集又被钉回首屏。「点了没变多」的那种只是本批到底了，照旧每批重置

复刻它的 fixture 必须**真的换页**（`location.href` 到 `/groups/<id>/posts/<pid>/`，
fixture 站点有对应的详情页路由）。**只把信息流 `display:none` 是复刻不出来的**：
`querySelectorAll` 照样取得到隐藏元素，不修也能通过（实测踩过）。回归
`test_a_fold_that_opens_the_post_detail_does_not_swallow_the_feed`：不修只采到
`{5601, 9004}`（详情页里那两条），修了三条主贴和信息流里的评论都在。
