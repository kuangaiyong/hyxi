...
description: hyxi 采集语义：Facebook 评论区的三处折叠必须点开、正文图抓取（tweakers + facebook_group）、全量重跑 force_full、老帖新回复如何避免被时间倒序埋掉。改采集脚本的增量/去重/翻页逻辑之前读这份。
---
...
3. **要循环点到不再增长**（评论分页），每轮之间照常 `humanDelay`：连着几十次点
   「加载更多」在行为分析里比翻页还扎眼。轮次上限 `COMMENT_FOLD_ROUNDS`，
   **撞上限必须打日志** —— 静默停在半路等于又一次少采，而这回连日志都不提
...
`scrollOnce()` 曾经是「滚一次 + 等 2.5 秒 + 比高度」。**信息流的懒加载比这慢**，
于是每轮都在第一批之后就宣布「页面不再增长，已到底」。真站实测（2026-09-13，
用户那份数据跑 force_full 一轮）：

```
[13:40:41]  点开评论折叠 12 处
[13:40:46]  批次 1：提取 8 条（含评论），新增 8 条
[13:40:48]  页面不再增长，已到底          ← 而这个小组库里已经攒了 115 条主贴
```

后果是**上面所有修复都只对首屏那几条生效**：同一轮点开了 12 处折叠，却只有 1 条主贴的
回复数突破了原来的上限（2 → 6）。现在改成分几次「推一把」+ 轮询等增长
（`SCROLL_NUDGES` × `SCROLL_GROWTH_MS`，真到底时不必白等满）。等待期间不发任何请求，
只是把「多久算没有下一批」放宽 —— 与「像一个有耐心的真实用户」的姿态一致。

复刻它的 fixture 有两条硬要求，写测试前先看，否则测出来是空转的（都实测踩过）：
