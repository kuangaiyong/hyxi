...
- **提升孤儿只清 `parent_fingerprint`，`reply_level` 一个字不动**。「显示在树根」和「它本身是主贴」是两回事：树位置由父指针决定（`build_tree()` 只看父指针），导出那条路的层级由 `order_by_thread()` 按深度重算 —— 所以留着层级不影响任何呈现，却让出口分得出「真主贴」和「丢了父贴的回复」。混为一谈的代价是静默的：后者的 `message_id` 是 **comment_id**，「🔗 原帖」会给它拼出 `/permalink/<comment_id>/`，点开既不是这条回复也不是它的主贴（评审在真实库上实测：65 条带链接的主贴里 2 条是这样）。**存量里已经被抹平的行只能靠 `force_full` 全量重跑修回来** —— 重采时 UPDATE 会把父指针和层级一起写回
...
`scripts/test_impact.py`：`select` 只打印选中了哪些、为什么；`run` 选完直接并行跑；`--paths 文件…` 不看 git 直接指定。
**改 `conftest.py`、`pytest.ini`、`scripts/test_impact.py`，或看不懂选测结论（为什么判全量、为什么没选中某条）之前，
...
**只给主贴，判据是 `reply_level` 而不是「排在树根」** —— 丢了父贴的回复也显示在树根，
但它的 `message_id` 是 comment_id，拿它拼出的链接点开谁也不是（见存储红线第七条）。
这类卡片的徽标也要跟着说实话（`ResultsView` 按 `reply_level > 0` 标成
「回复（主贴缺失）」）：它是**唯一没有原帖链接**的卡片，标成「主贴」只会让人以为
链接掉了。回归见 `e2e:link` 的场景二之二 —— **断言由出口数据驱动**（先从
`/posts` 里筛出 `reply_level > 0` 的行，再按 `#序号` 去页面上找那张卡片）；
反过来按徽标筛再断言，没有这种行时是恒真的，实测就那么空转通过过。
