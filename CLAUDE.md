...
实测证据（真 facebook.com 小组 `2407063016436085`）：2 批时 41 条 = 19 主贴 + 22 评论；3 批时 77 条，时间 / 作者 / `message_id` 仅 1 条为空（那是一条四个字段全空的 article，信息流里的广告/占位卡片），`message_id` 重复 0 组，**含展开/收起标记 0 条**；**保留结果再跑一次增量：提取 16 条、新增 0 条** —— 展开是确定性的，指纹跨进程稳定。

`tests/fixtures/login_site.py` 的小组页按上述结构复刻，连坑一起：文本为空的头像链接排在作者链接前面、主贴 `aria-label` 给相对时间、评论 `aria-label` 给一个时区错开的绝对时间、tooltip 靠 `mouseover` 现挂、第二条主贴的正文是折叠的（点「展开」才换成全文并把按钮变成「收起」）。
...
**167 个测试，必须全部 PASSED**（本机实测 `167 passed in 205s`）。修改任何核心逻辑后必须在仓库根目录运行：
